...
const AuthForm = ({ mode, apiEndpoint, redirectPath, otherPath, linkText }) => {
    ...
    const schema = z.object({
        name: mode === 'register' ? z.string().min(3, { message: "Name is required" }) : z.string().optional(),
    ...
    const handleSubmit = async (e) => {
        ...
        if (!result.success) {
            ...
            const role = redirectPath.includes('admin') ? 'admin' : 'user';
            const dataToSend = {
                email: formData.email,
                password: formData.password,
                ...(mode === 'register' && { name: formData.name, role }),
            };
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (token) {
                        ...
                        } else if (redirectPath.includes('user')) {
